...
void HypEthInterface::watchBaseBiosTable()
{
    auto BIOSAttrUpdate = [this](sdbusplus::message::message& m) {
        ...
        auto TOTAL_INTF_SUPPORTED = (manager.get().if1Exist()) ? 2 : 1;
...
{
    lg2::error("Setting IPv6 static default gateway");
    ...
    for (auto& addr : addrs)
    {
        ...
        if (ipObj->type() == HypIP::Protocol::IPv6)
        {
            if (ipObj->origin() == HypIP::AddressOrigin::Static)
            {
                ...
                // Update ipv6 gateway as well - this will trigger the BIOS table update
